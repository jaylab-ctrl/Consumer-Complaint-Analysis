...
- **Response Classification:** `company_response_to_consumer` (multi-class; e.g., *Closed with explanation*, *Closed with monetary relief*, *In progress*, etc.).
- **Sentiment Analysis:** polarity score over `consumer_complaint_narrative` (e.g., negative/neutral/positive or continuous polarity).
- **Time Series:** monthly complaint counts (overall and/or by product/state).  
**Normalization:** Unicode fixes, lowercasing, regex cleanup for artifacts, optional PII redaction for narratives, date parsing to monthly buckets.
